...
class AccountPayment(models.Model):
    ...
    def procesar_pago(self):
            ...
            for l in reg.invoice_id:
                ...
                if lista:
                    for mov in self.move_line_ids:
                        if mov.account_id.id==self.partner_id.property_account_receivable_id.id and round(mov.credit,2)==round(anticipo_completo,2):
        ...
        if round(pago_deuda,2)<=round(self.credito,2):
            ...
            raise ValidationError("Comuniquese con el administrador del Sistema{0} ".format(pago_deuda))
            ...
            if len(payment_term_line) >0:
                for l in payment_term_line:
                    ...
                    for x in invoice.contrato_estado_cuenta_ids:
                        if invoice.contrato_id.tasa_administrativa:
                            deuda_total+=(x.saldo_cuota_capital+x.saldo_seguro+x.saldo_rastreo+x.saldo_otros)
                        else:
                            deuda_total+=(x.saldo_seguro+x.saldo_rastreo+x.saldo_otros)
