...
class WizardImportDocuments(models.TransientModel):
    ...
    def import_xml_credit_note_and_invoice(self):
        ...
        for l in detalles:
            ...
            for impuesto in impuestos:
                obj_impuesto = self.env['account.tax'].search([('l10n_ec_code_base','=',impuesto['codigo'])])
                if obj_impuesto.id:
                    impuesto_ids.append(obj_impuesto.id)
                else:
    ...
    def import_xml_out_withholdings(self):
        ...
        for l in impuestos:
            ...
            obj_impuesto = self.env['account.tax'].search([('l10n_ec_code_base','=',l['codigo'])])
            dct_line={
                    'fiscal_year':infoRetencion['periodoFiscal'],
                    'tax_id':obj_impuesto.id,
