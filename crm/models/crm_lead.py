...
class WizardPagoCuotaAmortizacion(models.TransientModel):
    ...
    def _onchange_payment_method_id(self):
        if self.journal_id:
            list_method=[]
            self._cr.execute("""select inbound_payment_method from account_journal_inbound_payment_method_rel where journal_id={0}""".format(self.journal_id.id))
            res = self._cr.dictfetchall()
            for l in res:
                list_method.append(l['inbound_payment_method'])
            return {'domain': {'payment_method_id': [('id', '=', list_method)]}}
