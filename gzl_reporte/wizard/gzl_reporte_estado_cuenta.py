...
class ReporteEstadoCuenta(models.TransientModel):
    ...
    def obtener_listado_facturas(self,filtro,partner_id):
        ...
        sql=self.obtener_sql_de_listas(partner_id)
        query_final=sql + ' select * from lista_documentos {0} order by fecha_emision '.format(filtro)
        self.env.cr.execute(query_final)
    ...
    def obtener_sql_de_listas(self,partner_id):
        ...
        query_facturas=(""" select 
                                    ...
                                    am.amount_residual as monto_adeudado, 
                                    am.amount_total as monto_total, 
                                    am.ref as referencia, 
                                    am.narration as observaciones, 

                                    case    when am.type = ANY (array['in_invoice','out_invoice','in_debit','out_debit']) then am.amount_residual 
                                            else 0 end as debe ,


                                    case    when am.type = ANY (array['in_refund','out_refund']) then am.amount_residual 
                                            else 0  end as haber ,


                                    case    when am.type = ANY (array['in_refund','out_refund']) then  am.reversed_entry_nc_id ::VARCHAR
                                            when am.type = ANY (array['in_debit','out_debit']) then  am.debit_origin_nd_id :: VARCHAR
        ...
        query_retenciones=("""select 
                                    ...
                                    aj.name as documento_contable, 
                                    0 as monto_adeudado, 
                                    ar.amount_total as monto_total, 
        ...
        query_final='with lista_documentos as ( '+query_facturas+' union all ' +query_pagos +' union all '+query_retenciones +' )'
        ...
        for partner in lista_partner:
            ...
            if lista_facturas:
                ...
                for factura in lista_facturas:
                    ...
                    sheet.write(current_line, 7, factura['monto_adeudado'] , currency_format)
                    sheet.write(current_line, 8, factura['debe']  ,currency_format)
                    sheet.write(current_line, 9,factura['haber']  ,currency_format)
    ...
    def print_report_pdf(self):
        return self.env.ref('gzl_reporte.repote_estado_cuenta_pdf_id').report_action(self)
    ...
    def obtenerDatos(self,):
        ...
        for partner in lista_partner:
            ...
            for dct in lista_facturas:
                ...
                dct.pop('monto_total')
                dct['reglon']='detalle'
                dct['saldo']=partner['saldo_inicial']-dct['debe'] + dct['haber']
                lines.append(dct)
