...
class ReporteEstadoDeCuenta(models.TransientModel):
    ...
    def xslx_body(self, workbook, name):
        ...
        format_datos = workbook.add_format({'font_name':'Arial','font_size':  10,'align': 'left'})
        currency_format = workbook.add_format({'font_name':'Arial','font_size':  10,'num_format': '[$$-409]#,##0.00','text_wrap': True,'align':'center' })
        currency_format.set_align('vcenter')
        formato_cabecera_tabla = workbook.add_format({'font_name':'Arial','font_size':  10,'align':'center','bold':True, 'bottom':1, 'top':1,'text_wrap':True})
        formato_pie_tabla = workbook.add_format({'font_name':'Arial','font_size':  10,'align':'left','bold':True, 'bottom':1, 'top':1})
        date_format = workbook.add_format({'font_name':'Arial','font_size':  10,'num_format': 'yyyy-mm-dd', 'align': 'center','text_wrap': True })
        ...
        body = workbook.add_format({'align': 'center','font_name':'Arial','font_size':10,'align': 'left','border':0,'text_wrap': True})
        ...
        for col, head in enumerate(title_main):
            sheet.set_column('{0}:{0}'.format(chr(col + ord('A'))),9)
