    ...
    def xslx_body(self,workbook,name):
        ...
        for contrato_id in contrato_ids:
            ...
            if contrato.cliente.name=='GUALAN YAGUACHI NOE JESUS':
                pagos_finales=pagos_ids.filtered(lambda line:int(rec.payment_date.month)==int(mes) and int(rec.payment_date.year)==int(anio))

                raise ValidationError('{0}'.format(pagos_finales))
